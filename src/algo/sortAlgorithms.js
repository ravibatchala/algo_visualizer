...
function quickSort(array, start, end , animation) {
  let index;
  if (array.length > 1) {
    index = partition(array, start, end, animation);

    if (start < index - 1) {
      quickSort(array, start, index - 1 , animation);
    }

    if (index < end) {
      quickSort(array, index, end, animation);
    }
  }
   return animation;
}
...
function partition(array, start, end, animation) {
  ...
  let pivot = array[pivotIdx] , i = start, j = end;
  while (i <= j) {
    while (array[i] < pivot) 
      i++;
